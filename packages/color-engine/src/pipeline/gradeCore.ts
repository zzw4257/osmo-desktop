/**
 * The shared WGSL grade core — single source of the grading math.
 *
 * Composed into two shells:
 * - preview fragment shader (gradeShader.ts, samples texture_external)
 * - export compute shader (exportShader.ts, reads P010 planes; runs in
 *   native wgpu on the Rust side with the exact same string)
 *
 * Contract:
 * - group(0): Params uniform + curves texture (1024×6 r32float)
 * - group(2): input LUT 3d + creative LUT 3d + lut sampler
 * - group(1) is owned by the shell (video source / planes+output)
 * - `grade_pixel(raw, uv)` : encoded source RGB + normalized frame uv →
 *   graded display-encoded RGB (clamped 0..1)
 *
 * Design rules:
 * - Zero pipeline rebuilds: every op always executes; neutral parameters are
 *   mathematical identity — dragging a slider only rewrites the uniform.
 * - Op order here IS grade schema semantics (schemaVersion 1); keep in sync
 *   with grade/schema.ts.
 * - uniforms.ts mirrors the Params struct byte-for-byte; change together.
 *
 * Domains: linear Rec.709 working space; tonal/wheel/curve ops run in a
 * gamma-encoded grading domain (γ=2.4 via pow), like Resolve video levels.
 */
export const GRADE_CORE_WGSL = /* wgsl */ `
struct Params {
...
fn grade_pixel(raw: vec3f, uv: vec2f) -> vec3f {
  ...
  // 8 fixed bands (centers every 45°), triangle weights
  var hue_delta = 0.0;
  var sat_mul = 1.0;
  var lum_mul = 1.0;
  for (var b = 0u; b < 8u; b++) {
  ...
  var v2 = v + sh_tint * sh_w + hi_tint * hi_w;

  // ---- fade (lifted blacks, film print emulation) ----
  let f = P.fade * 0.01;
  v2 = mix(v2, v2 * 0.88 + vec3f(0.10), f);

  // ---- creative LUT ----
  if (P.creative_lut_enabled == 1u) {
    let lutted = sample_lut3d(creative_lut, clamp(v2, vec3f(0.0), vec3f(1.0)),
                              f32(textureDimensions(creative_lut).x));
    v2 = mix(v2, lutted, clamp(P.creative_lut_strength, 0.0, 1.0));
  }
  ...
  return clamp(v2, vec3f(0.0), vec3f(1.0));
}
`;
